...
def train():
    ...
    conv7 = conv_layer(conv6, 64,128, phase_train, act='relu', pool=False, name='conv7')
    conv8 = conv_layer(conv7, 128,128, phase_train, act='relu', pool=True, name='conv8')

    ####Fully Connected Layer############
    flattened = tf.reshape(conv8 , [-1, 4*4*128])
    fc1 = fc_layer(flattened , 4*4*128 ,3000,keep_prob , name = 'fc1' )
    ...
    writer = tf.summary.FileWriter('C:/Users/srampras/PycharmProjects/Handwriting/tensorflow_lg/train')
    validation_writer = tf.summary.FileWriter('C:/Users/srampras/PycharmProjects/Handwriting/tensorflow_lg/validation')
    ...
    for i in range(config.epoch):
        ...
        for batch_xt, batch_yt in ds.test.next_batch():
            ...
            good_test += accuracy_sumt.eval(
                feed_dict={x: batch_xt, y_: batch_yt, phase_train.name: False, keep_prob: 1.0})
