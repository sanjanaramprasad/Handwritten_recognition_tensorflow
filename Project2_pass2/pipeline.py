    ...
    def run(self):
        with self.output().open('w') as out_file:
            ...
            while (index < len(self.files_1) - 1):
                ...
                if (len(person_images) > 2):
                    ...
                    for i in range(0, len(person_images)):
                        ...
                        for j in range(i + 1, len(person_images)):
                            ...
                            list_of_matches.append((img,name))
                    shuffle(list_of_matches)
                    print len(list_of_matches)
                    counter_final += len(list_of_matches)
                    for img,name in list_of_matches:
                        #print name
                        if ( split_count_valid < int(0.1 * (len(list_of_matches)))):
                            split_count_valid += 1
                            img.save(os.path.join(self.dirs_1[2], name))
                        elif (split_count_test < int(0.1 * (len(list_of_matches)))):
                            split_count_test += 1
                ...
                index = next_index
            print("Total", counter_final)
            print ("FINAL AVERAGE: ", counter_final/1568)
...
class non_match_images(luigi.Task):
    ...
    def run(self):
        with self.output().open('w') as out_file:
            ...
            print("MIS MATCHES" , counter)
            out_file.write("Status : done")


class training_data_generator(luigi.Task):
    def output(self):
        return luigi.LocalTarget('done_generating')
    ...
    def run(self):
        with self.output().open('w') as out_file:
            ...
            for img in range(self.num_imgs):
                ...
                self.output_file_labels.flush()
                #########################################################################3
            self.output_file_images.close()
            self.output_file_labels.close()
            ### Images ##
            f_in = open(str(self.images_filename),'rb').read()
            f_out = gzip.open(str(self.images_filename) + '.gz', 'wb')
            f_in_labels = open(str(self.labels_filename),'rb').read()
...
class encode_data(luigi.Task):
    ...
    def run(self):
        with self.output().open('w') as out_file:
            print ("encoding_data")
            output_dir = os.getcwd()
            input_dir = 'C:/Users/srampras/PycharmProjects/Handwriting'
            training_dir =  input_dir+ '/' + config.training_dir
            testing_dir = input_dir+'/'+ config.testing_dir
            validation_dir =  input_dir+'/'+config.validation_dir
            ...
            yield encode(training_dir,'training-images-and-ubyte', 'training-labels-and-ubyte')
            yield encode(testing_dir,'testing-images-and-ubyte', 'testing-labels-and-ubyte')
            # yield encode(testing_dir, 'testing-labels-and-ubyte_20',  label="label")
            yield encode(validation_dir, 'validation-images-and-ubyte', 'validation-labels-and_ubyte')
