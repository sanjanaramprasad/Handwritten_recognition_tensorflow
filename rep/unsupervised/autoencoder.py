...
data = mnist_reader.read_data_sets('handwritten_and_preprocessed.gz')
print (data._images)
learning_rate = 0.01
...
n_hidden_3 = 128
n_hidden_4 = 64
n_input = 3136 # MNIST data input (img shape: 28*28)
...
with tf.Session() as sess:
    sess.run(init)
    total_batch = int(data._num_examples/batch_size)
    # Training cycle
    ...
    encode_decode = sess.run(
        y_pred, feed_dict={X: data._images[:examples_to_show]})
    # Compare original images with their reconstructions
    f, a = plt.subplots(2, 15, figsize=(15, 2))
    for i in range(examples_to_show):
        a[0][i].imshow(np.reshape(data._images[i], (56, 56)))
        a[1][i].imshow(np.reshape(encode_decode[i], (56, 56)))
    f.show()
    plt.draw()
